...
- (void)imageWithImageData:(NSData *)imageData{
    ...
    ///获取沙盒目录
    self.filePath=[ImagePath stringByAppendingPathComponent:[NSString stringWithFormat:@"maikejia%d.png",k]];
    [imageData writeToFile:self.filePath atomically:NO];
//    NSLog(@"写入沙盒%@",self.filePath);

    [_imgsArr addObject:[NSString stringWithFormat:@"camera%d.png",k]];
    [_imagePath addObject:self.filePath];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSURL * URL = [NSURL URLWithString:KURLNSString(@"Upload/upGoodsPic")];
        ...
        NSURLSessionUploadTask *upLoadTask = [manager uploadTaskWithRequest:request fromData:imageData progress:nil completionHandler:^(NSURLResponse *response, id responseObject, NSError *error)
                                              {
                                                  if (error) {
                                                      ...
                                                      NSLog(@"请求完成的结果是:%@",[dic objectForKey:@"msg"]);
//                                                      //用户默认设置
